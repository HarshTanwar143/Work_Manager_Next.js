...
const Home = () => {
  ...
  return (
    ...
    <section className="flex flex-col items-center justify-center text-center p-10 bg-transparent text-violet-800">
      ...
      <p className="mt-4 text-lg">Start managing your tasks efficiently.</p>
      <Link href={user?._id ? '/profile/user' : '/signup'}>
        <button className="mt-6 px-6 py-3 bg-violet-700 text-white text-lg font-semibold rounded-lg hover:bg-violet-600">{user?._id ? 'Go to Profile' : 'Sign Up'}</button>
      </Link>
    </section>
